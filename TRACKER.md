...
**Deferred to a later pass, not forgotten (see the risk register's "Deferred" rows):** `AuthGate.jsx`'s missing `.catch` on the profile fetch (a real stuck-forever-loading-spinner bug, now lower-risk since the orphaned-account fix above removes its most likely trigger, but not itself fixed yet), `ManageInvites.jsx`'s list-refresh silently failing with no retry affordance, a few low-impact error-message polish items, and the "just-revoked session isn't invalidated immediately" question (marked "Needs Verification" — depends on Supabase's actual JWT behavior, not decided by discussion alone).
