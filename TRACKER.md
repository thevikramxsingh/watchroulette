...
**Closed out — the small deferred items above (2026-07-06):** three "obvious fixes, no design decision needed" items, closed together:
- `ManageInvites.jsx`: a failed list refresh now has its own dedicated error state (separate from the invite/revoke form's own error/success state, so the two no longer bleed into each other) with a "Try again" button that re-runs the same fetch — no more relying on a full page reload.
- `authApi.js`'s `callAdminEndpoint`: was collapsing two different failure modes into one generic "Request failed" — a thrown network error (offline, DNS, request never reaching a server) now gets its own friendly message instead of an unhandled exception with a raw `TypeError`, and a non-JSON server response now includes the actual HTTP status code in its fallback message instead of a bare "Request failed" with nothing to go on.
- `LoginScreen.jsx`: `requestMagicLink` had no `try/catch` around it at all — a thrown network failure (as opposed to a normal `{ error }` result from Supabase) left the button stuck on "Sending…" forever with no recovery except a page reload. Now caught and shown as a normal recoverable error, same pattern as everywhere else in this app.

**Verification:** 1 new component test (`ManageInvites.test.jsx` — list load failure shows Try again, click recovers it), 1 new component test (`LoginScreen.test.jsx` — thrown network failure recovers instead of hanging). `callAdminEndpoint` itself stays untested directly, consistent with this codebase's own testing boundary (thin network wrappers aren't unit tested — only the pure judgment-call functions extracted from them are; its behavior is exercised indirectly through `ManageInvites.test.jsx`'s existing invite/revoke error-path tests, which mock it at the function boundary). `oxlint` (0 errors), full test suite, and `vite build` all clean in an isolated scratch copy with all platform bindings installed (this sandbox's own `node_modules` was installed on macOS and can't run `oxlint`'s native binding under Linux — a pre-existing environment quirk, not a regression). The one intermittent failure seen across full-suite runs (`ArenaGame.test.jsx`'s "beats the current high score" real-timer test) is the same pre-existing flake documented since Module 2 — confirmed unrelated by an isolated re-run (9/9 clean).

**Wheel badges: fixed size replaced with a dynamic one (2026-07-06):** the poster badges on the spin wheel were stuck at a single fixed size (15 viewBox units) regardless of how many movies were on the wheel — too small to read clearly, and there's no cap anywhere on pool size to make a fixed value safe at every count. New `computeBadgeSize()` (`badgeSize.js`, pure, unit tested) measures the tightest angular gap between adjacent badges each render and sizes every badge (uniformly, not per-badge) to fill it — clamped between the old size as a floor (never worse than before) and a ceiling ~45% bigger (never so large a 2-3 movie pool looks absurd). 7 new unit tests. `oxlint`, decision-engine suite (34/34), `vite build` clean in an isolated scratch copy. Committed `5c4ea30`.

**Note on visibility:** this only grows badges meaningfully for pools of roughly 8 movies or fewer — past that, the safety floor keeps them at the old size on purpose, to avoid overlap. If your real repo usually has more movies than that on the wheel at once, you won't see a visible change, which is the deliberate trade-off, not a bug — worth deciding later whether that trade-off is still the right one once you've actually seen it live at your real pool sizes.

**Real bug found live — Manage Invites "can't go back" (2026-07-06):** tapping the Manage Invites nav toggle a second time appeared to do nothing. Root cause: `AuthGate`'s profile-reload effect was keyed on the whole `session` object, and Supabase fires `onAuthStateChange` for background events too (a proactive token refresh, or a tab regaining focus/visibility after being backgrounded — both routine on a phone), each handing back a *new* session object for the same signed-in user. Reacting to those the same as a real sign-in re-ran `loadProfile`, which briefly shows `<LoadingScreen/>` in place of the whole app — unmounting `AppShell` and silently resetting its in-memory `view` state back to the default. From the outside this reads as "I tapped Manage Invites again and it's still showing" — really, the screen had already reset behind the scenes, and the tap was just re-opening it. Fixed by keying the effect on `session?.user?.id` and skipping the reload entirely when the already-loaded profile already matches that id; a genuine sign-in/sign-out/account-switch still reloads normally. 2 new regression tests in `AuthGate.test.jsx`. `oxlint`, full `src/app` suite (35/35), `vite build` clean in an isolated scratch copy. Committed `ecc722a` — **not yet pushed**, needs `git push origin main`.

**Real security gap found live — revoked accounts kept full access (2026-07-06):** you tested this directly: revoke someone, then confirm they can still add movies, spin, play Arena. Confirmed real. Root cause, verified against Supabase's own docs rather than assumed: `admin-revoke.js`'s ban blocks *future* sign-ins, but there is no way to invalidate an access token that's already been issued — Supabase (like any stateless-JWT system) has it stay valid until it naturally expires (the `exp` claim, ~1 hour by default), ban or no ban. Every RLS policy on `movie_repo`/`game_lobby`/`round_history` only ever checked `auth.role() = 'authenticated'`, which a revoked person's still-valid token satisfies fine — so revoking someone didn't actually stop them from doing anything for up to an hour. The token itself was never going to be the right layer to fix this at; the database is. New `is_active_member()` Postgres function (`security definer stable`, pinned `search_path` per Supabase's own hardening guidance) checks `profiles.revoked = false` for the calling user; every policy on those three tables now ANDs it in. This makes revocation immediate regardless of token validity — Postgres itself refuses the request, not just the app's own UI.

**Still needs you:** this is a live database change, same category as the earlier migrations — run the block under `schema.sql`'s `is_active_member()` migration comment (SQL Editor -> New query -> paste -> Run). Safe to run live: each drop+create is effectively instant, no window with zero policy in effect. Not committed to code yet in a way that changes app behavior on its own (it's pure SQL) — `schema.sql` itself is already updated and will be part of the next push.
